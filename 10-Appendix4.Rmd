...
tab3 <- "
Table: (\\#tab:tab-D-3) Extract of the `running` dataframe, after updating with each iteration through unique temperatures. Note the repeated appearance of patch 12, which is a common final destination patch.

| patch| temp| ftemp| dest| dest_temp|inter_patch | dest_ftemp| clim_conn|
|-----:|----:|-----:|----:|---------:|:-----------|----------:|---------:|
...
cat(tab3) # output the table in a format good for HTML/PDF/docx conversion
```
...
```{r fig-D-10, fig.cap= "(ref:cap-D-10)", fig.width = 25/2.54, fig.height = 11/2.54}
p1
```
