...
baseDir<-file.path("C:/Users",user,"Google Drive/PhD/Ch4/Methods/Sites/Main maps")
...
borneo<-readOGR(file.path("../Borneo countries"),"borneo") 
borneo_crop<-crop(borneo,extent(113,127,3.5,7))
sabah<-readOGR(baseDir,"sabah_outline")
danum<-readOGR(baseDir,"danum_area") 
danum_crop<-crop(danum,extent(117.59,117.98,5.12,4.9))
usmfr<-readOGR(baseDir,"logging_concession")
...
p1<-ggplot()+
  ...
  geom_text(aes(x=115,y=6.4,label="N"),size=3)+
  theme_classic()+
...
saveRDS(p4,file = file.path(outDir,"figB-sample-design.Rds"))

ggsave(plot = p4,
       filename = file.path(outDir,"figS1_sample_design.png"),
...
saveRDS(combi,file = file.path(outDir,"fig1_sample_design.Rds"))

ggsave(plot = combi,
       filename = file.path(outDir,"fig1_sample_design.png"),
       dpi=800,width = 16.6,height=9,units="cm")
ggsave(plot = combi,
       filename = file.path(outDir,"fig1_sample_design.pdf"),
       dpi=800,width = 16.6,height=9,units="cm")
...
p4<-ggplot()+
  geom_polygon(data = circle,aes(x,y),fill=cbPalette[1],
               alpha=0,colour="black")+
  geom_polygon(data = seg1,aes(x,y),fill=bwPalette[4])+
  geom_polygon(data = seg2,aes(x,y),fill=bwPalette[4])+
  geom_polygon(data = seg3,aes(x,y),fill=bwPalette[4])+
  geom_polygon(data = seg4,aes(x,y),fill=bwPalette[4])+
  geom_point(data = pts, aes(x,y,shape=distance),size=1.5)+
  geom_segment(aes(x = 0.5 , y = 0.5*lineGrad , 
                   xend = pts[1,1] - 0.5, 
                   yend = (pts[1,1] - 0.5)*lineGrad),
...
combi_bw <-
  ...
  draw_plot_label(label = c("(a)", "(b)", "(d)","(c)"),
                  ...
                  hjust = c(-0.5,0.3,-0.5,0.3),
                  vjust = c(1.7,1.7,1.4,1.4),
                  colour=c("black","white","black","black"))

ggsave(plot = combi_bw,
       filename = file.path(outDir,"bw/fig1_sample_design.png"),
       dpi=800,width = 16.6,height=9,units="cm")
ggsave(plot = combi_bw,
       filename = file.path(outDir,"bw/fig1_sample_design.pdf"),
       dpi=800,width = 16.6,height=9,units="cm")
