...
pred <- function(dat,df_name, results_df,
                 mod_family = "gaussian",
                 inc_TBA = TRUE,
                 RV, 
    ...
    if(vary_EV_class != "factor"){
        ...
        newdat_primary <- 
            newdat[newdat$forest_type == "Primary",]
        
        newdat_logged <- 
            newdat[newdat$forest_type == "Logged",]
    }
    ...
    if(mod_family == "binomial"){
        ...
        if(inc_TBA){
         ...
         } else mm <- model.matrix(~ forest_type,newdat)
...
plot_continuous <- function(pred_df, 
    ...
    if(facetting == "~ microhabitat"){
        p <- p + 
            facet_wrap(~ microhabitat, nrow = 1, scales = facet_scale)
        
    } else if (facetting == "day_night ~ microhabitat"){
        p <- p + 
            facet_grid(day_night ~ microhabitat, scales = facet_scale)
    } else if(facetting =="~ instrument"){
        p <- p + 
            facet_wrap(~ instrument, nrow = 1, scales = facet_scale)
    }
