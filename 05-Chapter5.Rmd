...
# Successful area results
load("data/ch5/models/patch_params/prop-success.Rdata")
success_area_results <- success_area_results$full_area_10_sep_2.results
# Cc loss results
...
# Raw ccc data
region_ccc <- readRDS("data/ch5/area_10_sep_2/region_ccc.Rds")
```

```{r median-cc-results, include = FALSE}
### Median cc
median_cc_interaction <- 
    data.frame(median_cc_results$interaction_results) %>% 
    mutate(RV = "median_cc",
           EV = "interaction",
           stat = "F",
           stat_val = F,
    ...
    mutate(ref = paste(stat, " = ", signif(stat_val,3), ", ",
                       ...
                       sep = ""))
median_cc_results <- 
    data.frame(median_cc_results$main_results) %>% 
    mutate(RV = "median_cc",
           EV = dropped_EV,
           stat = "F",
           stat_val = F,
...
median_cc_dat_2012 <-
    median_cc_dat %>% 
    filter(year == "2012" &
               df_name == "full_area_10_sep_2") %>% 
    arrange(median_cc) %>% 
    mutate(median_cc = signif(median_cc,3))

# Overall median (across everything)
median_cc_2012 <- unique(region_cc$median_cc_2012)
```

```{r prop-success-results, include = FALSE}

### Proportion connected area
prop_success_interaction <- 
    data.frame(success_area_results$interaction_results) %>% 
...
prop_success_results <-
    data.frame(success_area_results$main_results) %>% 
    mutate(RV = "prop_success",
           EV = dropped_EV,
           stat = "F",
           stat_val = F,
...
prop_success_dat_2012 <-
    ...
    filter(year == "2012" &
               df_name == "full_area_10_sep_2") %>% 
    arrange(prop_success_area) %>% 
    mutate(prop_success_area = signif(prop_success_area * 100,3))
prop_success_dat_yr <-
    success_area_dat %>% 
    group_by(realm) %>% 
    filter(df_name == "full_area_10_sep_2") %>%
    summarise(prop_success_diff = diff(prop_success_area)) %>% 
    mutate(prop_success_diff = (signif(prop_success_diff * 100, 3))) %>% 
    arrange(prop_success_diff)
```

```{r cc-loss-results, include = FALSE}

### Proportion cc loss
cc_loss_results <-
    cc_loss_results$main_results %>%
    mutate(RV = "cc_loss",
           EV = dropped_EV,
           Df = df,
